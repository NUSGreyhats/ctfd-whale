...
async function delete_container(container_id) {
    let response = await CTFd.fetch("/api/v1/plugins/ctfd-whale/admin/container?container_id=" + container_id, {
        method: "DELETE",
...
async function renew_container(container_id) {
    let response = await CTFd.fetch(
        "/api/v1/plugins/ctfd-whale/admin/container?container_id=" + container_id, {
        method: "PATCH",
        credentials: "same-origin",
        headers: {
            Accept: "application/json",
...
$(".renew-container").click(function (e) {
    ...
    CTFd.ui.ezq.ezQuery({
        ...
        success: async function () {
            await renew_container(container_id);
            location.reload();
        },
    });
});
