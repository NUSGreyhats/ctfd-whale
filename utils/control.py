...
class ControlUtil:
    ...
    def try_add_container(user_id, challenge_id, team_id=None):
        ...
        try:
            existing = DBContainer.get_current_containers(
                user_id=user_id,
                team_id=team_id,
    ...
    @staticmethod
    def try_remove_container(user_id, team_id=None, challenge_id=None):
        if team_id is None and is_team_mode():
            team_id = get_team_id_for_user(user_id)
        cache = CacheProvider(app=current_app)
        if not cache.acquire_global_lock():
    ...
    @staticmethod
    def try_renew_container(user_id, team_id=None, challenge_id=None):
        if team_id is None and is_team_mode():
            team_id = get_team_id_for_user(user_id)
    ...
    @staticmethod
    def try_renew_container_by_id(container_id):
    ...
    @staticmethod
    def try_remove_container_by_id(container_id):
        cache = CacheProvider(app=current_app)
        if not cache.acquire_global_lock():
            return False, 'Server busy, please retry.'
